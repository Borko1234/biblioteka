...
class LibraryManagementGUI:
    def __init__(self, master, filename):
        ...
        self.bg_avtor_entry.grid(row=8, column=1, sticky="ew")
        ...
        try:
            with open(self.filename, 'r') as file:
                for line in file:
                    ...
                    if len(parts) == 7:
                        kniga_ime, kniga_avtor, data_vpisvane, signatura, bg_avtor, chujd_avtor, nalichen_broi = parts
        ...
        with open(self.filename, 'w') as file:
            for loan in self.book_loans:
                ...
                if all(key in loan for key in
                       ['Име на книгата: ', 'Автор на книгата', 'Дата на вписване: ', 'Сигнатура', 'БГ автор', 'Чужд автор','Наличен брой']):
                    file.write(
                        f"{loan['Име на книгата: ']},{loan['Автор на книгата']},{loan['Дата на вписване: ']},{loan['Сигнатура']},"
                        f" {loan['БГ автор']},{loan['Чужд автор']},{loan['Наличен брой']}\n")
    ...
    def view_loans(self):
        loans = "\n".join(
            [
                f"Book: {loan['Име на книгата: ']}, Автор: {loan.get('Автор на книгата', '')}, Дата на вписване: {loan['Дата на вписване: ']}"
                f", Сигнатура: {loan['Сигнатура']}, БГ автор {loan['БГ автор']}, Чужд автор {loan['Чужд автор']},{loan['Наличен брой']}"
